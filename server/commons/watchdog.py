...
class WatchDog(threading.Thread):
        ...
        for application in room.applications:
            for c in room.applications[application]:
                ...
                if len(channel.users()) == 0 and len(channel.masters()) == 0:
                    if int(current_time - channel.last_action) > \
                           SETTINGS.ROOM_EMPTY_TIME:
                        room.remove(channelName=channel.name,
                                    appName=application)
                        Log().add('[i] WatchDog: Deleted %s channel' % (
                            channel.name), 'red')
    ...
    def checkHttpClient(self, current_time, client):
        if client.type is 'http' and \
               int(current_time - client.last_action) > \
               SETTINGS.WATCHDOG_HTTP_DISCONNECT_TIME:
            return False
        return True
    ...
    def removeClients(self, total_client):
        ...
        if len(clients) != total_client:
            ...
            Log().add('[i] WatchDog: Server hosts %d clients' % (
                len(clients)), 'yellow')
        for (uid, client) in clients:
            if self.checkHttpClient(current_time, client) is False:
                uidToDelete.append(client.unique_key)
            elif int(current_time - client.last_action) > \
                     SETTINGS.WATCHDOG_MAX_IDLE_TIME:
                uidToDelete.append(client.unique_key)
        if len(uidToDelete) > 0:
            Log().add('[i] WatchDog: Deleted %d clients' % (
                len(uidToDelete)), 'yellow')
            self.lock.acquire()
